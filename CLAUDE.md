...
- **A price refresh (`performRefreshPrices`, behind "Refresh Prices"/"Refresh All Prices" and the daily snapshot cron) also updates `dividend_frequency` — TMX-listed tickers only.** TMX's GraphQL response includes a `dividendFrequency` field that already matches the app's own `Monthly`/`Quarterly`/`Semi-Annual`/`Annual` strings exactly (whitelisted against `INTERVAL_MONTHS` in `upsertStockInfo` regardless, rather than trusting TMX to never change its wording). Yahoo (US tickers) has no equivalent field, so those are untouched by refresh and still rely on the manual edit modal or `POST /api/stocks/backfill-frequency`. Unlike `backfill-frequency` (which only fills a blank `dividend_frequency`, `WHERE ... IS NULL`), a refresh **overwrites even a value you set by hand** — that's deliberate, so it stays in sync with TMX going forward. TMX returning `null` (no dividend on file) does *not* clear an existing value, since a null response is more often a transient API miss than a confirmed "dividend stopped."
...
- `DIVIDEND` (cash) accumulates `dividends_paid`; `DIVIDEND_REINVEST` instead adds shares and buy cost and is **not** counted in `dividends_paid`. A broker (e.g. Wealthsimple) records a DRIP as two separate transactions — the cash dividend, then a separate reinvestment purchase, often a day or more later — so a `DIVIDEND_REINVEST`'s `total` is the same money as an already-counted `DIVIDEND`, not new income; summing both (the old behavior) double-counted every DRIP dividend on the Portfolios and Dividends pages. `DIVIDEND_REINVEST`'s effect on `return` still comes through via the raised cost basis (`buy_total`) and the extra shares' market value — it just isn't income on its own. `CONTRIBUTION`/`WITHDRAWAL`/`TRANSFER_IN`/`TRANSFER_OUT`/`BUY`/`SELL`/`DIVIDEND`/`DIVIDEND_REINVEST` all auto-adjust `cash_balance` via `CASH_BALANCE_DELTA` — a BUY spends `total + commission`, a SELL nets `total - commission`, a `DIVIDEND` adds `total` (cash landing in the account, on top of the existing `dividends_paid` bump), and a `DIVIDEND_REINVEST` subtracts `total` (that cash immediately spent buying the additional shares, on top of the existing share/buy-cost bump). `CONTRIBUTION`/`WITHDRAWAL`/transfers/`DIVIDEND_REINVEST` can drive `cash_balance` negative (e.g. a transfer logged before its matching contribution, or a reinvest logged without its own preceding `DIVIDEND` row) — that's allowed by design, not validated against — but a **BUY is rejected with 400 if `total + commission` exceeds the portfolio's current `cash_balance` (plus `CASH_EPSILON` tolerance)**, in `POST /api/transactions`, the `PUT` edit route (which first reverses the row's own old cash effect before checking, so re-editing an existing BUY doesn't reject itself), and the CSV import (tracked as a running per-portfolio balance the same way the oversell guard tracks running shares) — `DIVIDEND_REINVEST` has no equivalent affordability guard. `POST /api/import` (the full-backup JSON restore) is exempt — it writes each portfolio's `cash_balance` directly from the payload rather than deriving it from `CASH_BALANCE_DELTA`.
