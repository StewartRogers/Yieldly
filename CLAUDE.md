...
- `DIVIDEND` (cash) accumulates `dividends_paid`; `DIVIDEND_REINVEST` instead adds shares and buy cost. `CONTRIBUTION`/`WITHDRAWAL`/`TRANSFER_IN`/`TRANSFER_OUT`/`BUY`/`SELL` auto-adjust `cash_balance` via `CASH_BALANCE_DELTA` — a BUY spends `total + commission`, a SELL nets `total - commission`. `DIVIDEND`/`DIVIDEND_REINVEST` deliberately don't touch it (a cash dividend lives in `dividends_paid` instead, and a reinvest converts straight to shares with no cash ever landing in the account). `CONTRIBUTION`/`WITHDRAWAL`/transfers can still drive `cash_balance` negative (e.g. a transfer logged before its matching contribution) — that's allowed by design, not validated against — but a **BUY is rejected with 400 if `total + commission` exceeds the portfolio's current `cash_balance` (plus `CASH_EPSILON` tolerance)**, in `POST /api/transactions`, the `PUT` edit route (which first reverses the row's own old cash effect before checking, so re-editing an existing BUY doesn't reject itself), and the CSV import (tracked as a running per-portfolio balance the same way the oversell guard tracks running shares). `POST /api/import` (the full-backup JSON restore) is exempt — it writes each portfolio's `cash_balance` directly from the payload rather than deriving it from `CASH_BALANCE_DELTA`.
