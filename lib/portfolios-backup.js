 ...
 * deployment. It persists portfolio names/codes/order/cash_balance only (not
 * the transaction ledger) and relies on a writable local filesystem. On Vercel the
...
function makePortfoliosBackup(db, backupPath) {
  return async function backup() {
    try {
      const rows = await db.all('SELECT name, code, display_order, cash_balance FROM portfolios ORDER BY display_order, id');
      await fs.promises.writeFile(backupPath, JSON.stringify(rows, null, 2));
    } catch (e) {
  ...
  try {
    ...
    try {
      for (const p of saved) {
        await tx.execute({
          sql: 'INSERT INTO portfolios (name, code, display_order, cash_balance) VALUES (?, ?, ?, ?)',
          args: [p.name, p.code, p.display_order || 0, p.cash_balance || 0],
        });
      }
      await tx.commit();
      committed = true;
    } catch (e) {
      // Only roll back if commit didn't already finalize the tx.
