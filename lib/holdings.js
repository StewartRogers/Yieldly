...
// dividends_paid counts cash DIVIDEND only, not DIVIDEND_REINVEST. A broker
// (e.g. Wealthsimple) records a DRIP as two separate transactions — the cash
// dividend, then a separate reinvestment purchase, often a day or more later
// — so DIVIDEND_REINVEST's total is the same money as an already-counted
// DIVIDEND, not new income. Summing both double-counted every DRIP dividend.
// DIVIDEND_REINVEST's effect on `return` still comes through correctly via
// buy_total (below) raising the cost basis by the reinvested amount, and via
// market_value on the extra shares it bought — it just isn't income on its
// own.
const HOLDINGS_SQL = `
    SELECT
      ...
      MIN(CASE WHEN t.type IN ('BUY','DIVIDEND_REINVEST') THEN t.date END) AS first_buy_date,
      s.market_price,
      s.dividend_yield,
